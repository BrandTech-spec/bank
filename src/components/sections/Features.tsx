    ...
    <section>
      <Element name="features">
        <div className="container  text-white">
          <div className=" flex  flex-col  border-2 border-s3 items-center justify-center rounded-7xl  max-md:flex-col feature-after max-md:border-none max-md:rounded-none max-md:gap-3">
            {features.map(({ id, icon, caption, title, text, button }) => (
              <div
                key={id}
                className=" z-2 md:px-10 px-5 md:pb-10 pb-5 m max-md:border-2 max-md:border-s3 max-md:rounded-3xl "
              >
            ...
            <ul className="relative flex justify-around flex-grow px-[5%] border-2 border-s3 rounded-7xl max-md:hidden">
              ...
              {details.map(({ id, icon, title }) => (
                <li key={id} className="relative pt-16 px-4 pb-14">
                  ...
                  <h3 className="relative z-2  mx-auto my-0 base-small text-center uppercase">
                    {title}
                  </h3>
                </li>
              ))}
